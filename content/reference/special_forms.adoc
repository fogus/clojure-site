...
The map __binding-form__ to the right of the `&` in `destr` is an associative destructuring __binding-form__ detailed above. The presence of `:keys` and `:or` directives fill in the specifics of the incoming bindings, including `xargs1` and `xargs2`, the latter bound to `42` by default. Calling `destr` to supply keyword arguments is as follows:
...
That is, having declared `destr` to accept keyword arguments means that the __rest-args__ accept any combination of, and any ordering of the key/values associated with the keys `:xargs1` and `:xargs2`. As expected, key/value pairs not supplied on invocation bind to `nil` unless a default was supplied as in the case of `:xargs2`.
