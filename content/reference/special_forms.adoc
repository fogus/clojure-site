...
Defines a function (fn). Fns are first-class objects that implement the https://clojure.github.io/clojure/javadoc/clojure/lang/IFn.html[IFn interface]. The `IFn` interface defines an `invoke()` function that is overloaded with arity ranging from 0-20. A single fn object can implement one or more invoke methods, and thus be overloaded on arity. One and only one overload can itself be variadic, by specifying the ampersand followed by a single _rest-param_. Such a variadic entry point, when called with arguments that exceed the positional params, will find them in a binding form defined in the rest param. Should this binding form be a name then the value will be a seq, otherwise if destructuring form is found then binding are constructed accordingly. However, if the supplied args do not exceed the positional params, the rest param will be `nil`.
...
=== Sequence binding destructuring

Vector __binding_form__s sequentially unpack values in collections like vectors, lists, seqs, strings, arrays, and anything that supports https://clojure.github.io/clojure/clojure.core-api.html#clojure.core/nth[`nth`] to names. The basic sequential form is a vector of __binding-form__s, which will be bound to successive elements from the __init-expr__, looked up via `nth`. In addition, and optionally, a __binding-form__ following a `&` will be  bound to the remainder of the sequence, i.e. that part not yet bound, and looked up via https://clojure.github.io/clojure/clojure.core-api.html#clojure.core/nthnext[`nthnext`].
...
It is often the case that you will want to bind same-named symbols to the map keys. The `:keys` directive addresses the redundancy often found in the binding __binding-form-key__ pairs:
...
Clojure provides a way to define functions that accept keyword arguments in the __next-param__s of a function call. To define a function that accepts keyword arguments you supply a map destructuring form in the __next-param__ declaration position. For example, consider a function that takes a sequence and optional keyword arguments and returns a string comprised of the values given:

[source,clojure]
----
(defn destr [arg & {:keys [xarg1 xarg2] :or {xarg2 42} :as opts}]
  [arg xarg1 xarg2 opts])
----

The map __binding-form__ to the right of the `&` in `destr` is an associative destructuring __binding-form__ detailed above. Calling `destr` to supply keyword arguments is as follows:

[source,clojure]
----
(destr 0 :xarg1 1 :xarg2 2)
->[0 1 2 {:xarg1 1, :xarg2 2}]

(destr 0 :xarg1 1)
->[0 1 42 {:xarg1 1}]
----

That is, having declared `destr` to accept keyword arguments means that the __rest-args__ accept any combination of, and any ordering of the key-values associated with the keys `:xargs1` and `:xargs2`. The function `destr` above is functionally equivalent to the following declaration of `destr2`:

[source,clojure]
----
(defn destr2 [arg & kvs]
  (let [{:as opts} kvs
        xarg1 (get opts :xarg1)
        xarg2 (get opts :xarg2 42)]
    [arg xarg1 xarg2 opts]))
...
Indeed, the `fn` macro will transform the body of a function taking keyword arguments into roughly that of `destr2`. As previously mentioned in the section about associative destructuring, map binding forms mechanically create bindings via look-up in associative forms. However, in `destr2` you may have noticed that the __binding-form__ `{:as opts}` received the sequence `kvs` resulting in the map `opts`. While sequences are not associative structures, they are treated as such on the right-hand side of a map binding form where the components of the sequence are alternating key-value pairs.

=== Nested destructuring

Destructuring forms nest because a destructuring form may take the place of a binding name to further drill into component structures. Take a simple case where an unnested data structure is destructured to retrieve a user name:

[source,clojure]
----
(let [{user :user} {:user "Fred"}]
  user)
-> "Fred"
----

If the map changes such that the user name is nested inside of a component map then this code will need to change to accomodate the nesting. To do so you can replace the user binding with a destructuring form that digs into the nested map:

[source,clojure]
----
(let [{{user :user} :info} {:id 42, :info {:username "Fred"}}]
  user)
-> "Fred"
----

This is the general pattern for nested destructure binding — placing nested destructure bindings in positions where __binding-form__s would appear. This works for nestings of arbitrary depth and of mixed maps and sequences.
