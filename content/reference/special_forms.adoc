...
The simplest binding form in Clojure is a symbol:
...
Clojure also supports abstract structural binding called destructuring in `let` binding lists, `fn` parameter lists, and by extension any macro that expands into a `let` or `fn`. A _binding-form_ is a data structure literal containing symbols that are bound to the respective parts of the value provided to it (referred to as __init-expr__ in the Clojure documentation). Destructure bindings are abstract in that a vector literal binds to any sequential data structure, while a map literal binds to an associative data structure, or equivalently, a sequence of alternating key/value pairs.

Binding symbols that don’t match their respective part due to an absence of data (too few elements in a sequential structure, no key in an associative structure, etc) bind to `nil`.
...
Finally, also optionally, `:as` followed by a symbol will cause that symbol to be bound to the entire __init-expr__:
...
->{:first-thing 1, :second-thing 2, :third-thing 3, :rest-things (4 5 6 7), :all-things [1 2 3 4 5 6 7]}
----
...
It is often the case that you will want to bind same-named symbols to the map keys. The `:keys` directive allows you to avoid the redundancy often found in the binding __binding-form-key__ pairs:
...
Clojure provides a way to define functions that accept keyword arguments in the __next-param__s of a function call. To define a function that accepts keyword arguments you supply a map destructuring form in the __next-param__ declaration position. For example, consider a function that takes a sequence and reurns a string of its elements representing a version number:

[source,clojure]
----
(defn version [coll & {:keys [pre post sep] :or {sep "."}}]
  (str pre
...
The map binding form to the right of the `&` in `version` is exactly the same shown above. The presense of `:keys` and `:or` directives fill in the details of the incoming bindings, including the names `pre`, `post`, and `sep`, the latter bound to `"."` by default. Calling `version` to supply keyword parameters is as follows:
...
(version [1 10 3] :pre "v")
-> "v1.10.3"

(version [1 10 3] :pre "v" :post "-SNAPSHOT")
-> "v1.10.3-SNAPSHOT"
----

That is, having declared `version` to accept keyword arguments means that the rest args accept any combination of, and any ordering of the key/values associated with the keys `:pre`, `:post`, and `:sep`. As expected, key/value pairs not supplied on invocation bind to `nil` unless a default was supplied as in the case of `:sep`.
...
This is the general pattern for nested destructure binding — placing nested destructure bindings in positions where binding names would appear. This works for nestings of arbitrary depth and of mixed maps and sequences.
