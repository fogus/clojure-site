...
The map __binding-form__ to the right of the `&` in `glom` is exactly the same shown above. The presense of `:keys` and `:or` directives fill in the details of the incoming bindings, including `pre`, `post`, and `sep`, the latter bound to `"."` by default. Calling `glom` to supply keyword parameters is as follows:
