...
[source,clojure]
----
(defn glom [coll & {:keys [pre post sep] :or {sep "."}}]
  (str pre
...
The map __binding-form__ to the right of the `&` in `glom` is an associate destructuring __binding-form__ detailed above. The presence of `:keys` and `:or` directives fill in the specifics of the incoming bindings, including `pre`, `post`, and `sep`, the latter bound to `"."` by default. Calling `glom` to supply keyword arguments is as follows:
